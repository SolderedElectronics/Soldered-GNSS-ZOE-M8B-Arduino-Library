...
void loop()
{
    ...
    if (myGNSS.getEsfInfo()) // Poll new ESF STATUS data
    {
        ...
        for (uint8_t i = 0; i < myGNSS.packetUBXESFSTATUS->data.numSens; i++)
        {
            ...
            Serial.print(i);
            Serial.print(F("   ")); // Print the sensor number

            // Print the sensor type
            Serial.print(sensorStatus.sensStatus1.bits.type);
            if (sensorStatus.sensStatus1.bits.type < 10)
                Serial.print(F(" "));
            Serial.print(F("  "));

            Serial.print(sensorStatus.sensStatus1.bits.used);
            Serial.print(F("  ")); // Print the used flag
            Serial.print(sensorStatus.sensStatus1.bits.ready);
            Serial.print(F("  ")); // Print the ready flag

            Serial.print(sensorStatus.sensStatus2.bits.calibStatus);
            Serial.print(F("  ")); // Print the calibration status
            Serial.print(sensorStatus.sensStatus2.bits.timeStatus);
            Serial.print(F("  ")); // Print the time status

            Serial.print(sensorStatus.faults.bits.badMeas);
            Serial.print(F("  ")); // Print the bad measurement flag
            Serial.print(sensorStatus.faults.bits.badTTag);
            Serial.print(F("  ")); // Print the time tag flag
            Serial.print(sensorStatus.faults.bits.missingMeas);
            Serial.print(F("  "));                            // Print the missing measurement flag
