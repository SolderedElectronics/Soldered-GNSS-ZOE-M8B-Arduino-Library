...
void setup()
{
    ...
    if (myGNSS.setAutoESFALGcallback(&printESFALGdata) ==
        true) // Enable automatic ESF ALG messages with callback to printESFALGdata
        Serial.println(F("setAutoESFALGcallback successful"));

    if (myGNSS.setAutoESFINScallback(&printESFINSdata) ==
        true) // Enable automatic ESF INS messages with callback to printESFINSdata
        Serial.println(F("setAutoESFINScallback successful"));

    if (myGNSS.setAutoESFMEAScallback(&printESFMEASdata) ==
        true) // Enable automatic ESF MEAS messages with callback to printESFMEASdata
        Serial.println(F("setAutoESFMEAScallback successful"));

    if (myGNSS.setAutoESFSTATUScallback(&printESFSTATUSdata) ==
