...
void newRAWX(UBX_RXM_RAWX_data_t ubxDataStruct)
{
    ...
    for (uint8_t block = 0; block < ubxDataStruct.header.numMeas; block++) // For each block
    {
        ...
        if (ubxDataStruct.blocks[block].gnssId < 100)
            Serial.print(F(" ")); // Align the gnssId
        if (ubxDataStruct.blocks[block].gnssId < 10)
            Serial.print(F(" ")); // Align the gnssId
        Serial.print(ubxDataStruct.blocks[block].gnssId);
        Serial.print(F("  SV ID: "));
        if (ubxDataStruct.blocks[block].svId < 100)
            Serial.print(F(" ")); // Align the svId
