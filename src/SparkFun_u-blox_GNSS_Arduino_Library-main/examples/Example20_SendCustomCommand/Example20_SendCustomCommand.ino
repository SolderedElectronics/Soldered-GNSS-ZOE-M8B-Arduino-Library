...
void setup()
{
    ...
    uint8_t customPayload[MAX_PAYLOAD_SIZE]; // This array holds the payload data bytes. MAX_PAYLOAD_SIZE defaults to
                                             // 256. The CFG_RATE payload is only 6 bytes!

    // setPacketCfgPayloadSize tells the library how many bytes our customPayload can hold.
    // It is more memory-efficient to call setPacketCfgPayloadSize before .begin (to avoid creating a new buffer,
    // copying across the contents of the old buffer and then deleting the old buffer). But let's call it here just to
    // prove that we can.
    ...
    myGNSS.setAutoPVT(
        true); // Enable AutoPVT. The module will generate measurements automatically without being polled.
