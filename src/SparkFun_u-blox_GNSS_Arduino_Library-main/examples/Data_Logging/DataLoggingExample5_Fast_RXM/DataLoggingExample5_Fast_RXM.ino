...
// On the Artemis, we can disable the internal I2C pull-ups too to help reduce bus errors
    ...
    am_hal_gpio_pincfg_t sclPinCfg =
        g_AM_BSP_GPIO_IOM4_SCL; // Artemis MicroMod Processor Board uses IOM4 for I2C communication
    ...
    // myGNSS.enableDebugging(Serial, true); // Or, uncomment this line to enable only the important GNSS debug messages
    // on Serial

    myGNSS.disableUBX7Fcheck(); // RAWX data can legitimately contain 0x7F, so we need to disable the "7F" check in
                                // checkUbloxI2C
    ...
    // (This will also disable any "auto" messages that were enabled and saved by other examples and reduce the load on
    // the I2C bus)
    ...
    // Modules like the ZED-F9P can produce RAW navigation data at rates of up to 25Hz but not while using all of the
    // GNSS constellations. Please consult the data sheet for the Performance figures for your module. In this example
    // we make sure GPS is enabled and then disable Galileo, GLONASS, BeiDou, SBAS and QZSS to achieve 25Hz.
    myGNSS.enableGNSS(
        true, SFE_UBLOX_GNSS_ID_GPS); // Make sure GPS is enabled (we must leave at least one major GNSS enabled!)
    ...
    // Produce 7 navigation solutions per second. That's a lot of RAWX data - especially when using both GPS bands L1
    // and L2. The SD library and card need to be able to cope with the data rate too. You may need a faster SD library
    // to go above 7Hz.
    myGNSS.setNavigationFrequency(7);

    myGNSS.setAutoRXMSFRBX(true,
                           false); // Enable automatic RXM SFRBX messages: without callback; without implicit update
...
void loop()
{
    ...
    while (myGNSS.fileBufferAvailable() >=
           sdWriteSize) // Check to see if we have at least sdWriteSize waiting in the buffer
    {
        digitalWrite(LED_BUILTIN, HIGH); // Flash LED_BUILTIN each time we write to the SD card

        uint8_t myBuffer[sdWriteSize]; // Create our own buffer to hold the data while we write it to SD card

        myGNSS.extractFileBufferData(
            (uint8_t *)&myBuffer,
            sdWriteSize); // Extract exactly sdWriteSize bytes from the UBX file buffer and put them into myBuffer

        myFile.write(myBuffer,
                     sdWriteSize); // Write exactly sdWriteSize bytes from myBuffer to the ubxDataFile on the SD card
    ...
    {
        Serial.print(
            F("The number of bytes written to SD card is: ")); // Print how many bytes have been written to SD card
        Serial.println(bytesWritten);

        uint16_t maxBufferBytes =
            myGNSS.getMaxFileBufferAvail(); // Get how full the file buffer has been (not how full it is now)

        // Serial.print(F("The maximum number of bytes which the file buffer has contained is: ")); // It is a fun thing
        // to watch how full the buffer gets Serial.println(maxBufferBytes);
    ...
    if (Serial.available()) // Check if the user wants to stop logging
    {
        ...
        uint16_t remainingBytes =
            myGNSS.fileBufferAvailable(); // Check if there are any bytes remaining in the file buffer
        ...
        while (remainingBytes > 0) // While there is still data in the file buffer
        {
            ...
            myGNSS.extractFileBufferData(
                (uint8_t *)&myBuffer,
                bytesToWrite); // Extract bytesToWrite bytes from the UBX file buffer and put them into myBuffer

            myFile.write(myBuffer,
                         bytesToWrite); // Write bytesToWrite bytes from myBuffer to the ubxDataFile on the SD card
        ...
        Serial.print(F(
            "The total number of bytes written to SD card is: ")); // Print how many bytes have been written to SD card
        Serial.println(bytesWritten);

        uint16_t maxBufferBytes =
            myGNSS.getMaxFileBufferAvail(); // Show how full the file buffer has been (not how full it is now)
