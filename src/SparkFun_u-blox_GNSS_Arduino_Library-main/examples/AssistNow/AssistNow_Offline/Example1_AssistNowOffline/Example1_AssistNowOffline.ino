...
const char getPeriod[] =
    "period=1;"; // Optional. The number of weeks into the future that the data will be valid. Can be 1-5. Default = 4.
const char getMgaResolution[] =
    "resolution=1;"; // Optional. Data resolution: 1 = every day; 2 = every other day; 3 = every 3rd day.
// Note: always use resolution=1. findMGAANOForDate does not yet support finding the 'closest' date. It needs an exact
// match.
...
void setup()
{
    ...
    snprintf(theURL, URL_BUFFER_SIZE, "%s/%s%s%s%s%s%s%s%s", assistNowServer, getQuery, tokenPrefix, myAssistNowToken,
             tokenSuffix, getGNSS, getFormat, getPeriod, getMgaResolution);
    ...
    // Uncomment the next line to enable the 'major' debug messages on Serial so you can see what AssistNow data is
    // being sent
    ...
    if (payloadSize > 0)
    {
        if (getLocalTime(&timeinfo))
        {
            ...
            todayStart = myGNSS.findMGAANOForDate(payload, (size_t)payloadSize, timeinfo.tm_year + 1900,
            ...
            {
                Serial.println("Could not find the data for today. This will not work well. The GNSS needs help to "
                               "start up quickly.");
            }

            // Find the start of tomorrow's data
            tomorrowStart = myGNSS.findMGAANOForDate(payload, (size_t)payloadSize, timeinfo.tm_year + 1900,
                                                     timeinfo.tm_mon + 1, timeinfo.tm_mday, 1);
        ...
        {
            Serial.println(
                "Failed to obtain time. This will not work well. The GNSS needs accurate time to start up quickly.");
        }
    }
    ...
    if (getLocalTime(&timeinfo)) // Get the local time again, just to make sure we are using the most accurate time
    {
        ...
        myGNSS.setUTCTimeAssistance(timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour,
    ...
    if (payloadSize > 0)
    {
        ...
        // Push the AssistNow data for today. Don't use UBX_MGA_ACK_DATA0's. Use the default delay of 7ms between
        // messages.
        ...
        myGNSS.pushAssistNowData(todayStart, true, payload, tomorrowStart - todayStart, SFE_UBLOX_MGA_ASSIST_ACK_YES,
                                 100);
