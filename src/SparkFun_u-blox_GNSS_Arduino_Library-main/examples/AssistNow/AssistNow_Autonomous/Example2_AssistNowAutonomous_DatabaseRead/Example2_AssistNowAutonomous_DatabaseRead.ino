...
void setup()
{
    ...
    Serial.println(
        F("AssistNow Autonomous data collection is in progress. Press any key to quit and read the database."));
    ...
    //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // Read the AssistNow Autonomous database from the module and pretty-print it (so it can be copied and pasted into
    // the next example)
...
#define MAX_DATABASE_LENGTH 32768 // Allocate 32kBytes to store the navigation database
    ...
    Serial.println(F("Storage has been allocated for the database."));
    Serial.flush();
