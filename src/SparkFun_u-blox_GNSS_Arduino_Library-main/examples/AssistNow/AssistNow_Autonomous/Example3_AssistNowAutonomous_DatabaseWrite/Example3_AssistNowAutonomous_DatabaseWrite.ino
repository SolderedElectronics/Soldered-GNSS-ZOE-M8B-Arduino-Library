/*
  ...
  All SparkFun boards have battery-backup for the RAM which will means the database is retained if you disconnect the
  power. The module will use the database data from the battery-backed RAM when you turn the power back on. You will
  only see the improvement in the time-to-first-fix if you disable the battery first - or you are using a non-SparkFun
...
void setup()
{
    ...
    if (bytesPushed != databaseLen)
        Serial.println(F("Warning: bytesPushed does not match databaseLen! Maybe the database contains bad data? Or "
