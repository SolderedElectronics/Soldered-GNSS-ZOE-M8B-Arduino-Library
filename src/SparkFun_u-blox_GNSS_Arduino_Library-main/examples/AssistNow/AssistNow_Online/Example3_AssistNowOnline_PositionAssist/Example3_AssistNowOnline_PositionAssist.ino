...
const char useLatitude[] =
    "lat=55.0;"; // Use an approximate latitude of 55 degrees north. Replace this with your latitude.
const char useLongitude[] =
    "lon=-1.0;";                  // Use an approximate longitude of 1 degree west. Replace this with your longitude.
...
void setup()
{
...
#ifdef USE_SERVER_ASSISTANCE
    snprintf(theURL, URL_BUFFER_SIZE, "%s/%s%s%s%s%s%s%s%s%s%s", assistNowServer, getQuery, tokenPrefix,
             myAssistNowToken, tokenSuffix, getGNSS, getDataType, useLatitude, useLongitude, useAlt, usePosAcc);
#else
    snprintf(theURL, URL_BUFFER_SIZE, "%s/%s%s%s%s%s%s", assistNowServer, getQuery, tokenPrefix, myAssistNowToken,
             tokenSuffix, getGNSS, getDataType);
#endif
    ...
    if (getLocalTime(&timeinfo))
    {
        ...
        myGNSS.setUTCTimeAssistance(timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour,
                                    timeinfo.tm_min, timeinfo.tm_sec, 0, 2, 0, 0, SFE_UBLOX_MGA_ASSIST_ACK_YES, 100);
    }
    else
    {
        Serial.println(
    ...
    // Use 55 degrees (*10^7) north, 1 degree (*10^7) west, 100m (10000cm) altitude, 100km (10000000cm) accuracy.
    // Replace these with your position. The units for lat and lon are degrees * 1e-7 (WGS84) The units for alt (WGS84)
    // and posAcc (stddev) are cm.
