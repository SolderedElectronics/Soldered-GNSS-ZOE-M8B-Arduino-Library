...
void loop()
{
    ...
    if (millis() - lastTime > 1000)
    {
        ...
        // getHighResLatitudeHp: returns the high resolution component of latitude from HPPOSLLH as an int8_t in degrees
        // * 10^-9 getHighResLongitude: returns the longitude from HPPOSLLH as an int32_t in degrees * 10^-7
        // getHighResLongitudeHp: returns the high resolution component of longitude from HPPOSLLH as an int8_t in
        // degrees * 10^-9 getElipsoid: returns the height above ellipsoid as an int32_t in mm getElipsoidHp: returns
        // the high resolution component of the height above ellipsoid as an int8_t in mm * 10^-1 getMeanSeaLevel:
        // returns the height above mean sea level as an int32_t in mm getMeanSeaLevelHp: returns the high resolution
        // component of the height above mean sea level as an int8_t in mm * 10^-1 getHorizontalAccuracy: returns the
        // horizontal accuracy estimate from HPPOSLLH as an uint32_t in mm * 10^-1
        ...
        // To allow this example to run on standard platforms, we cheat by converting lat and lon to integer and
        // fractional degrees
