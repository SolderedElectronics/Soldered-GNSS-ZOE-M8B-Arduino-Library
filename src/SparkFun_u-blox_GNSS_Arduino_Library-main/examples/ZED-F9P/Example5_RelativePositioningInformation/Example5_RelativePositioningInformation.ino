...
void loop()
{
    // From v2.0, the data from getRELPOSNED (UBX-NAV-RELPOSNED) is returned in UBX_NAV_RELPOSNED_t
    // packetUBXNAVRELPOSNED Please see u-blox_structs.h for the full definition of UBX_NAV_RELPOSNED_t You can either
    // read the data from packetUBXNAVRELPOSNED directly or can use the helper functions: getRelPosN/E/D;
    // getRelPosAccN/E/D
    ...
    for (int i = 0; i < 500; i++)
    {
        ...
        if (numBytes > 0) // Check if data was received
        {
            ...
            // myGNSS.pushRawData(((uint8_t *)&store), numBytes, true); // Push the RTCM data via I2C - always use stops
            // on long RTCM pushes

            myGNSS.pushRawData(((uint8_t *)&store),
                               numBytes); // Push the RTCM data via I2C - using restarts to break up long I2C pushes
