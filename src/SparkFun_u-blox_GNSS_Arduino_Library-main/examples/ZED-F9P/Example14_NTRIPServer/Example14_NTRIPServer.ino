...
void setup()
{
    ...
    myGNSS.setI2COutput(COM_TYPE_UBX | COM_TYPE_NMEA |
                        COM_TYPE_RTCM3); // UBX+RTCM3 is not a valid option so we enable all three.
    ...
    // See this tutorial on getting PPP coordinates:
    // https://learn.sparkfun.com/tutorials/how-to-build-a-diy-gnss-reference-station/all
    response &=
        myGNSS.setStaticPosition(-128020830, -80, -471680384, -70, 408666581, 10); // With high precision 0.1mm parts
    if (response == false)
    {
    ...
    // Because setting an incorrect static position will disable the ability to get a lock, we will skip saving during
    // this example if (myGNSS.saveConfiguration() == false) //Save the current settings to flash and BBR
...
void beginServing()
{
    ...
    while (Serial.available() == 0)
    {
        ...
        if (ntripCaster.connected() == true)
        {
            ...
            // This is the main sending loop. We scan for new ublox data but processRTCM() is where the data actually
            // gets sent out.
            while (1)
            {
                if (Serial.available())
