 ...
 *
 * @brief        Use ESP32 WiFi to get RTCM data from Swift Navigation's Skylark caster as a Client, and transmit GGA
 *using a callback By: SparkFun Electronics / Nathan Seidle & Paul Clark Date: January 13th, 2022 License: MIT. See
 *license file for more information but you can basically do whatever you want with this code.
 *
 *               This example shows how to obtain RTCM data from a NTRIP Caster over WiFi and push it over I2C to a
 *ZED-F9x. It's confusing, but the Arduino is acting as a 'client' to a 'caster'. In this case we will use Skylark. But
 *you can of course use RTK2Go or Emlid's Caster too. Change secrets.h. as required.
 *
 *               The rover's location will be broadcast to the caster every 10s via GGA setence - automatically using a
 *callback.
 *
 *               This is a proof of concept to show how to connect to a caster via HTTP and show how the corrections
 *control the accuracy.
 *
 *               It's a fun thing to disconnect from the caster and watch the accuracy degrade. Then connect again and
 *watch it recover!
 *
...
#else
#include "base64.hpp" // library from https://github.com/Densaugeo/base64_arduino, will work with any platform
#endif
...
const unsigned long maxTimeBeforeHangup_ms =
    10000UL; // If we fail to get a complete RTCM frame after 10s, then disconnect from caster

bool transmitLocation = true; // By default we will transmit the unit's location via GGA sentence.

WiFiClient
    ntripClient; // The WiFi connection to the NTRIP server. This is global so pushGGA can see if we are connected.
...
void pushGPGGA(NMEA_GGA_data_t nmeaData)
{
    ...
    if ((ntripClient.connected() == true) && (transmitLocation == true))
    {
        ...
        Serial.print(
            (const char *)nmeaData.nmea); // .nmea is printable (NULL-terminated) and already has \r\n on the end
...
void setup()
{
    ...
    myGNSS.setPortInput(COM_PORT_I2C,
                        COM_TYPE_UBX | COM_TYPE_NMEA |
                            COM_TYPE_RTCM3); // Be sure RTCM3 input is enabled. UBX + RTCM3 is not a valid state.

    myGNSS.setDGNSSConfiguration(
        SFE_UBLOX_DGNSS_MODE_FIXED); // Set the differential mode - ambiguities are fixed whenever possible
    ...
    myGNSS.setAutoPVTcallback(&printPVTdata); // Enable automatic NAV PVT messages with callback to printPVTdata so we
                                              // can watch the carrier solution go to fixed

    // myGNSS.saveConfiguration(VAL_CFG_SUBSEC_IOPORT | VAL_CFG_SUBSEC_MSGCONF); //Optional: Save the ioPort and message
    // settings to NVM
...
bool beginClient()
{
    ...
    else
    {
        ...
        else
        {
            ...
            encode_base64(encodedCredentials, userCredentials, strlen(userCredentials)); // Note: Input array is
                                                                                         // consumed
#endif
        ...
        // Serial.print(F("Caster responded with: ")); Serial.println(response); // Uncomment this line to see the full
        // response
